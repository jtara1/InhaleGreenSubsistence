...
onready var game_state_manager = Env.find_node_of_type(GameStateManager)


func _ready():
	._ready()
	gravity = 0
	game_state_manager.connect("game_won", self, "_on_GameStateManager_game_won")
...
func _on_SpaceShip_body_entered(body):
	if not is_lifting_off:
		if body is Agent:
			game_state_manager.win_the_game()
